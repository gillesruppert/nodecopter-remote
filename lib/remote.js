...
var Remote = exports = module.exports = function Remote (pins, anims) {
  ...
  button1.on('up', function () {
    if (typeof anims[0] !== 'string') return;
    if (ledAnimations.indexOf(anims[0]) !== -1) {
      this.emit('animateLeds', anims[0], 50, 5);
    } else if (animations.indexOf(anims[0]) !== -1) {
      this.emit('animate', anims[0], 500);
    }
  }.bind(this));

  button2.on('up', function () {
    if (typeof anims[1] !== 'string') return;
    if (ledAnimations.indexOf(anims[1]) !== -1) {
      this.emit('animateLeds', anims[1], 50, 5);
    } else if (animations.indexOf(anims[1]) !== -1) {
      this.emit('animate', anims[1], 500);
    }
  }.bind(this));
